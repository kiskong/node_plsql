...
function start(serverConfig)
{
	...
	for (i = 0; i < serverConfig.server.static.length; i++) {
		text = 'Serving static files for mount path "' + serverConfig.server.static[i].mountPath + '" from "' + serverConfig.server.static[i].physicalDirectory + '"';
		if (debug.enabled) {
			debug(text);
		}
		if (serverConfig.suppressOutput === false) {
	...
	text = 'The host ' + host + ' is now listening on port ' + serverConfig.server.port + '.';
	if (debug.enabled) {
		debug(text);
	}
	if (serverConfig.suppressOutput === false) {
