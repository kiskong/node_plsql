...
function connect(services) {
	...
	var text = 'Connect with Oracle as ' + services[0].username;
	if (services[0].database.length > 0) {
		text += '@' + services[0].database;
	}
	text += '...';
	console.log(text);

	// Create Oracle binding object
	var oracle = new oracleBindings.OracleBindings(services[0]);
	...
	for (i = 0; i < services.length; i++) {
		installCode(oracle, services[i].username, services[i].password);
	}
