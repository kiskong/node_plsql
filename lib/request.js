...
* @api public
*/
function processRequestSync(serverConfig, service, req, res)
{
	'use strict';
	...
	debug('Invoking "' + req.params.name + '" with the arguments "' + JSON.stringify(args) + '...');
	var page = oracle.invokeProcedureSync(service.oracleSession, req.params.name, args, cgi);
	if (debug.enabled) {
...
function processRequest(serverConfig, service, req, res)
{
	...
	oracle.invokeProcedure(service.oracleSession, req.params.name, args, cgi, function (err, page) {
...
module.exports = {
	processRequestSync: processRequestSync,
