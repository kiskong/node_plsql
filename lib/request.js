...
function processRequest(serverConfig, oracleSession, username, password, req, res)
{
	...
	if (req.hasOwnProperty('files') && underscore.keys(req.files).length > 0) {
		debug('Request has files: ' + util.inspect(req.files, {showHidden: false, depth: null, colors: true}));
	...
	oracle.invoke(oracleSession, username, password, req.params.name, args, cgi, files, function pageReceived(err, page) {
