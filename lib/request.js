...
function pageProcessSuccess(res, page/*, err*/) {
	...
	// trace
	log.trace('C) A STORED PROCEDURE RETURN A PAGE (request.pageProcessSuccess)', page);
	...
	headerAndBody = header.getHeaderAndBody(page);
...
function pageProcessError(res, err) {
	...
	// trace
	log.trace('C) A STORED PROCEDURE REPORTED AN ERROR (request.pageProcessError)', err);
...
function reportRequestError(err, req) {
	var request = {
		headers: req.headers,
		url: req.url,
		method: req.method,
...
function processRequest(app, serverConfig, service, req, res) {
	...
	// trace
	log.trace('A) A REQUEST WAS RECEIVED (request.processRequest)', _.pick(req, ['_startTime', 'body', 'complete', 'cookies', 'files', 'headers', 'httpversion', 'method', 'originalUrl', 'params', 'query', 'signedCookies', 'url']));
	...
	// Prepare on object only containing the actual arguments
	args = _.extend(args, getParameter(req));
