...
function installCode(oracle, username, password) {
	...
	try {
		...
		log.error('Error when trying to install the procedural code', e);
...
function connect(config) {
	...
	for (i = 0; i < config.services.length; i++) {
		installCode(databaseHandle.oracle, config.services[i].databaseUsername, config.services[i].databasePassword);
	}
