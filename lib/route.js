...
var processRequest = function (app, serverConfig, service, req, res) {
	...
	request.processRequest(serverConfig, app.oracleSession, service, req, res);
};
...
function createPageRoute(app, serverConfig, service) {
	...
	text = 'Create page route for "' + path + '"';
	if (debug.enabled) {
		debug(text);
	} else {
		console.log(text);
...
function createDefaultPageRoute(app, serverConfig, service) {
	...
	text = 'Create default page route for "' + path + '" to "' + service.defaultPage + '"';
	if (debug.enabled) {
		debug(text);
	} else {
		console.log(text);
